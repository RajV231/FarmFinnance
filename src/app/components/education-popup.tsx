        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 animate-fade-in">
            <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden">
                <div className="bg-purple-600 p-4 text-white flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="font-bold text-xl">Krishi Guru Quiz</h2>
                        <p className="text-purple-200 text-sm">Earn ₹{quiz.reward.toLocaleString()} for correct answers!</p>
                    </div>
                </div>
                ...
                <div className="p-6">
                        ...
                        <div className={`p-4 rounded-xl mb-6 ${isCorrect ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                            ...
                            {isCorrect && (
                                <div className="mt-3 flex items-center gap-1 text-green-700 font-bold bg-green-200 w-fit px-3 py-1 rounded-full text-sm">
                                    <Coins className="w-4 h-4"/> +₹{quiz.reward} Added to Savings
                                </div>
                            )}
                        </div>
                    )}
                    ...
                    {showResult && (
                        <button 
                            onClick={handleClose}
                            className="w-full bg-gray-800 text-white py-3 rounded-xl font-bold hover:bg-gray-900 transition-colors"
                        >
                            Continue
                        </button>
                    )}
