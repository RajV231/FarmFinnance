...
import { Sprout, User, FileText, ShoppingCart, Target, Landmark, LogOut, FileSignature } from 'lucide-react';
    ...
    <div className="h-full flex flex-col bg-game-bg p-6 animate-fade-in relative overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
            <div className="bg-white rounded-xl shadow-sm p-4 border-l-4 border-yellow-500">
                 <div className="text-gray-500 text-xs font-bold uppercase">{t('credit_score')}</div>
            ...
            <div className="bg-white rounded-xl shadow-sm p-4 border-l-4 border-game-primary">
                 <div className="text-gray-500 text-xs font-bold uppercase">{t('season')}</div>
        ...
        <div className="space-y-3 mb-20 md:mb-0 md:grid md:grid-cols-2 md:gap-4 md:space-y-0">
            ...
            >
                <Sprout className="w-6 h-6" /> {t('start_season')}
            </button>
            ...
            >
                <Target className="w-6 h-6" /> {t('goals')}
            </button>
            ...
            >
                <Landmark className="w-6 h-6" /> {t('bank')}
            </button>

            <button 
                onClick={() => dispatch({ type: 'GO_TO_SCHEMES' })} 
                className="bg-white p-4 rounded-xl shadow-sm font-medium text-sky-600 flex flex-col items-center gap-2 border-2 border-transparent hover:border-sky-200 transition-all"
            >
                <FileSignature className="w-6 h-6" /> {t('schemes')}
            </button>
            ...
            >
                <ShoppingCart className="w-6 h-6" /> {t('shop')}
            </button>
            ...
            >
                <LogOut className="w-5 h-5" /> {t('quit')}
            </button>
        </div>
    </div>
  );
